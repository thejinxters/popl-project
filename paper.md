...
An interface class can have an empty generic function as well as a full executable function without the @abstract decorator. @abstract decorator creates an empty generic function that could be used within an interface class, but also could be used outside the class. For instance, in the **Code block I** IWriteMapping is an interface that has a __setitem__ generic function with @abstract decorator as well as an update method with a layout of its implementation.
